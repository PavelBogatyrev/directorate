...
angular.module('bodApp.view_revenue_lobs_pl_coe', ['ngRoute'])

    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider.when('/view_revenue_lobs_pl_coe', {
            templateUrl: 'screens/view_revenue_lobs_pl_coe/view_revenue_lobs_pl_coe.html',
            controller: 'ViewRLPCoeCtrl'
        });
    }])

    .controller('ViewRLPCoeCtrl', ['$scope', '$routeParams', '$http', 'ConfigurationService', 'LabelService', '$log',
        function ($scope, $routeParams, $http, ConfigurationService, LabelService, $log) {
            ...
            var fadeInListener = function () {
                ...
                $http.get(url + "api/lobsRevenueLeft").then(function (response) {
                    widgets.forEach(function (widget) {
                        if (widget.id == "widget_50_1") {
                            widget.view.setData(response.data[0]);
                            widget.view.getConfig().bar.padding = 0.1;
                            widget.view.render();
                            widget.view.fadeIn();

                        } else if (widget.id == "widget_50_3") {
                            widget.view.setData(response.data[1]);
                            widget.view.getConfig().bar.padding = 0.1;
                            widget.view.render();
                            widget.view.fadeIn();
